...
export class KeysComponent implements OnInit {
  ...
  refreshClusterDetails() {
    ...
    this.http.get(`${environment.api.uri}/api/cluster/details?name=${this.clusterName}`)
  ...
  refreshKeys() {
    ...
    this.http.get(`${environment.api.uri}/api/cluster/listKeys?name=${this.clusterName}&pattern=${this.pattern}`)
      .map((res: Response) => res.json())
      .subscribe((result: any) => {
        this.keys = result;
      }, (err: Error) => {
  ...
  onClick_Key(key: string) {
    ...
    this.http.get(`${environment.api.uri}/api/node/getkey?name=${this.clusterName}&key=${key}`)
